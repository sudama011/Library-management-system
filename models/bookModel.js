...
var createBook = (book, callback) => {
    var date = new Date();
    var sql = "INSERT INTO books (user_id,genre,title,author,publisher,edition,isbn,pages,date_issued) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
    db.executeQuery(sql, [null, book.genre, book.title, book.author, book.publisher, book.edition, book.isbn, book.pages, date], function(result) {
        callback(result);
    });
};
...
var delete_issue_date = (book_id, callback) => {
    var sql = "DELETE FROM issue_date WHERE book_id =?";
    db.executeQuery(sql, [book_id], function (result) {
        callback(result);
...
var totalBorrowed30 = (callback) => {
    ...
    var sql = "SELECT books.*, issue_date.book_id FROM issue_date INNER JOIN books ON issue_date.book_id=books.book_id WHERE (issue_date.date BETWEEN ? AND ?)";
